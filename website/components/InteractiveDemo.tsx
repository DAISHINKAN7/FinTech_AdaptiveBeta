...
export default function InteractiveDemo() {
  ...
  const result = useMemo<SimResult>(() => runSimulation(params), [params]);
  ...
  const insight = useMemo(() => {
    if (params.vixThreshold < 20) {
      return "Low VIX threshold: strategy goes defensive very frequently. Lots of min-variance days → lower volatility but also lower upside capture.";
    } else if (params.vixThreshold > 32) {
      return "High VIX threshold: only extreme crises (like COVID at VIX 80) trigger the override. ADANI and rate hikes may slip through without protection.";
    } else if (params.betavolPercentile < 60) {
      return "Low betavol percentile: frequent rebalances → higher transaction cost drag (~0.08% per event). Good adaptability, but cost accumulates over 6 years.";
    } else if (params.betavolPercentile > 80) {
      return "High betavol percentile: conservative triggering. Very few rebalances → low cost, but slower to adapt to changing regimes.";
    ...
    return `Balanced configuration: VIX override at ${params.vixThreshold} captures major crises, ${params.betavolPercentile}th percentile threshold triggers ${result.rebalanceCount} rebalances over 10 years.`;
  }, [params, result.rebalanceCount]);
      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-5">
        ...
        <div className="bg-navy-800/80 border border-navy-700 rounded-xl p-5 space-y-5">
          ...
          <ParamSlider
            ...
            presets={[
              { label: "Sensitive (20)", value: 20 },
              { label: "Default (25)",   value: 25 },
              { label: "Conservative (32)", value: 32 },
            ]}
          />
          ...
          <ParamSlider
            ...
            presets={[
              { label: "Reactive (45)",  value: 45 },
              { label: "Default (55)",   value: 55 },
              { label: "Conservative (75)", value: 75 },
            ]}
          />
              ...
              <div>
                ...
                <p className="text-xs text-gray-600 mt-0.5">
                  {activeCrisis
                    ...
                    : "AdaptiveBeta vs NIFTY50 Buy & Hold · 2015–2024"}
                </p>
              </div>
              {activeCrisis && (
                <button

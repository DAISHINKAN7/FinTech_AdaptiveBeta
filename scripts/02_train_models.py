...
What this script does:
  ...
  2. Trains XGBoost baseline + computes SHAP feature importance
  3. Trains Kalman filter betas for all stocks
  4. Trains the LSTM model (primary deep model)
  5. Fits the HMM regime classifier (3 states: bull/bear/transition)
...
def train_kalman(fast_mode: bool):
    ...
    sr = sr.loc[common]; mr = mr.loc[common]
    sr = sr.fillna(0)  # pre-IPO zeros
    ...
    # Estimate Kalman MAE vs 60d betavol target
...
def train_lstm_model(X_train_sc, y_train, X_test_sc, y_test,
    ...
    logger.info("LSTM parameters: %d", n_params)
...
def train_hmm():
    ...
    mr  = np.log(nifty_df["Close"] / nifty_df["Close"].shift(1)).dropna()
    vx  = vix_df["Close"].reindex(mr.index).ffill()
    ...
    ax.set_title("HMM Market Regime Classification (3 states)")
    ax.legend()
    plt.tight_layout()
...
def main():
    ...
    # ---- Model A: XGBoost ----
    if not args.no_xgb:
        _, _, xgb_mae, xgb_dir = train_xgboost(
    ...
    logger.info("Betavol threshold (Q%.0f of train): %.4f", THRESHOLD_QUANTILE * 100, threshold)
    ...
    compare = pd.DataFrame({
        "Model":         ["Static OLS Beta",  "Kalman Filter",  "XGBoost", "LSTM (proposed)"],
        "MAE":           [round(maes.get("Static OLS (baseline)", np.nan), 4),
                          round(maes.get("Kalman Filter", np.nan),          4),
                          round(maes.get("XGBoost", np.nan),                4),
                          round(maes.get("LSTM (proposed)", np.nan),        4)],
        "Direction_Acc": [np.nan, np.nan,
                          round(dir_acc.get("XGBoost", np.nan),            3),
                          round(dir_acc.get("LSTM (proposed)", np.nan),    3)],
        "Notes":         ["60d rolling OLS", "State-space EM",
                          "Gradient boosting + SHAP", "Seq2One LSTM + LayerNorm"],
    })
    ...
    logger.info("Saved signal_config.json: threshold=%.4f", threshold)

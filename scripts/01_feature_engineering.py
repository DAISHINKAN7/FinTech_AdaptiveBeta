...
What this script does:
  ...
  7. Saves everything to features/

Run:
    python scripts/01_feature_engineering.py

Outputs (all in features/):
    stacked_features.csv        — full cross-sectional feature matrix (~49× rows)
...
# ---------------------------------------------------------------------------
# Path setup — run from project root OR from scripts/
# ---------------------------------------------------------------------------
...
def read_csv(rel_path: str, date_col: str = "date") -> pd.DataFrame:
    """Load a CSV, set DatetimeIndex, sort ascending."""
    full = DATA_DIR / rel_path
    if not full.exists():
        raise FileNotFoundError(
            f"\n\n  ❌  Data file not found: {full}\n"
            f"  Put your CSV at: {full}\n"
            f"  See README.md → 'Where to put your data'\n"
        )
...
def align(df_or_series, trading_days: pd.DatetimeIndex, cols=None):
...
def main() -> None:
    ...
    if missing:
        logger.warning("Tickers not found in prices file (will be skipped): %s", missing)
    prices = align(prices_raw[available], TRADING_DAYS)

    # Market
    nifty_close = align(nifty_raw[["Close"]], TRADING_DAYS)["Close"]
    vix_raw     = read_csv("market/india_vix.csv")
    vix         = align(vix_raw[["Close"]], TRADING_DAYS)["Close"].rename("india_vix")

    # Macro
    usdinr = align(read_csv("macro/usdinr.csv")[["Close"]], TRADING_DAYS)["Close"].rename("usdinr")
    crude  = align(read_csv("macro/crude_oil.csv")[["Close"]], TRADING_DAYS)["Close"].clip(lower=0).rename("crude")
    # Negative crude on 2020-04-20 (WTI) clipped to 0
    ...
    # Flows (monthly → forward-fill to daily)
    ...
    # Sector indices
    bank = align(read_csv("sector/nifty_bank.csv")[["Close"]], TRADING_DAYS)["Close"].rename("nifty_bank")
    nit  = align(read_csv("sector/nifty_it.csv")[["Close"]], TRADING_DAYS)["Close"].rename("nifty_it")
    fmcg = align(read_csv("sector/nifty_fmcg.csv")[["Close"]], TRADING_DAYS)["Close"].rename("nifty_fmcg")
    ...
    # Late-IPO tickers: fill NaN returns (pre-IPO) with 0
    for t in LATE_IPO_TICKERS:
        if t in stock_ret.columns:
            pre_ipo_nans = stock_ret[t].isna().sum()
            stock_ret[t] = stock_ret[t].fillna(0)
            logger.info("  %s: filled %d pre-IPO NaN returns with 0", t, pre_ipo_nans)
    ...
    # -----------------------------------------------------------------------
    logger.info("\n[4/7] Computing rolling betas (%d windows × %d stocks)...",
    ...
    # PRIMARY PREDICTION TARGET: 20-day FORWARD beta volatility (60d base)
    # Row t → betavol observed at t + TARGET_HORIZON
    ...
    logger.info("  ✓  stacked_features.csv  (%d rows × %d cols)", *stacked.shape)
...
def _save_overview_plot(betas, betavols, vix_s, market_ret, idx) -> None:
    """Save a 4-panel diagnostic chart."""
    ...
    ax.set_title("RELIANCE.NS — Rolling Beta"); ax.legend(fontsize=8)

    ax = axes[0, 1]
    for w, c in zip([30, 60, 120], colors):
        betavols[w]["RELIANCE.NS"].plot(ax=ax, color=c, lw=1, label=f"BetaVol {w}d")
    ax.set_title("RELIANCE.NS — Beta Volatility"); ax.legend(fontsize=8)

    ax = axes[1, 0]
    vix_s.plot(ax=ax, color="#D85A30", lw=1)
    ax.axhline(25, color="yellow", ls="--", lw=0.8, label="VIX=25")
    ax.set_title("India VIX"); ax.legend(fontsize=8)
